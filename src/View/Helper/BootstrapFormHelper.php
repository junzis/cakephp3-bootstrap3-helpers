...
class BootstrapFormHelper extends FormHelper {
    ...
    **/
    public function create($model = null, Array $options = array()) {
        ...
        $this->templates([
            ...
            // Adding the template for textarea
            'textarea' => '<textarea class="form-control" {{attrs}} name="{{name}}" id="{{name}}">{{value}}</textarea>',
            'select' => '<select class="form-control" name="{{name}}"{{attrs}}>{{content}}</select>',
            'selectMultiple' => '<select class="form-control" name="{{name}}[]" multiple="multiple"{{attrs}}>{{content}}</select>',
            // Remove the checkbox and radio template
    ...
    **/
     ...
     *
    **/
    // This  FUNCTION is BROKEN, generates warnings. Removed.
    // public function label($fieldName, $text = NULL, array $options = []) {
    //
    //     $input_str = isset($options['input']) ? $options['input'] : '';
    //
    //     $nolabel = strpos($input_str, 'type="checkbox"') !== false
    //         || strpos($input_str, 'type="radio"') !== false  ;
    //     if ($nolabel) {
    //         return $text ;
    //     }
